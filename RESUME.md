...
- **W5 — BrightWave overlay re-score (staged, not yet run).** Full instructions in `NEXT_SESSION_PROMPT.md`. Returned sources land in `7_research_requests/<brief>/sources/`; **we synthesize the conclusions ourselves — BrightWave's own conclusions are NOT authoritative.** Path: sources → verified evidence ledgers → synthesized `market_overlay` (by us) → a **NEW additive** `4_pipeline/_w5_overlay.py` that reads the W4 output and writes `master_targets_ranked_overlay.csv` (W4 baseline left untouched for comparison). Modular, cited, re-runnable.
...
Confidence-aware composite, weights dict `W`: **size_fit 25, succession 20, geo 15, independence 15, operating_health 15, relevance 10** (sum 100). FMCSA fields unified census→agent→none per target. A-list = `confirmed_private` 'yes'; bench = 'unsure'; 'no' dropped. **Overlay hook (W5):** a NEW `4_pipeline/_w5_overlay.py` reads the W4 output and adds an **additive** overlay (separate weight set — points on top of the W4 composite; W4 dict unchanged, no renormalize), joining a synthesized `market_overlay` table by `sub_sector`/`scale_tier`/`geo` (derived from `da_school`/`da_special_needs`/`fleet_buses`/`state`) + per-target factors (EV/bus, owner-age=NOW−founded, driver-market, etc.). Baseline left intact. **Geo note:** W4 baseline `EXP` = 6 states (no UT); apply the UT priority bonus in the overlay, not by re-running the baseline.
